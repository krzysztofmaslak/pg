...
def list():
    ...
    orders = order.ioc.new_order_service(order.logger).find_by_page(user.account, int(request.args.get('page')))
    order.logger.debug('['+request.remote_addr+'] returning %s orders'%(len(orders)))
    count = order.ioc.new_order_service(order.logger).find_paid_orders_count(user.account)
