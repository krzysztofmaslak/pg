...
def login():
    ...
    login_blueprint.logger.info('['+request.remote_addr+'] Processing authentication request')
    ...
    if user is not None and user.active:
        if check_password_hash(user.password, request.json['password']):
            login_blueprint.logger.info('['+request.remote_addr+'] Authentication successful')
            session['username'] = username
            return Response(status=200, mimetype='application/json')
        else:
            error_message = messages.get_text(lang, 'login_wrong_username_or_password')
            login_blueprint.logger.info('['+request.remote_addr+'] Authentication failed password mismatch')
    else:
        if user is None:
            error_message = messages.get_text(lang, 'login_wrong_username_or_password')
        else:
            error_message = messages.get_text(lang, 'login_user_inactive')
        login_blueprint.logger.info('['+request.remote_addr+'] Authentication failed no user for username:'+username)
